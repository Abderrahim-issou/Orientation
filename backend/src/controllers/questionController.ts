...
export const getQuestionsByDiagnostic = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const diagnostic = await Diagnostique.findOne({
      diagnostique: diagnosticName,
    });
    ...
    if (!diagnostic) {
      throw new ApiError(
        404,
        `Diagnostic with name ${diagnosticName} not found`
      );
    }

    // Find all questions for this diagnostic
    const questions = await Question.find({
      diagnostique: diagnostic.diagnostique,
    });
